...
object ReplaceAbsentNodesPartitionAssignmentStrategySpec : Spek({
    ...
    describe("6-node kafka cluster with one test topic, replication factor=2, 3 racks") {
        val healthy = KafkaPartitionAssignment(1, listOf(
                Partition("test", 1, listOf(1, 2), listOf(1, 2), 1),
                Partition("test", 2, listOf(1, 3), listOf(1, 3), 1),
                Partition("test", 3, listOf(2, 3), listOf(2, 3), 2),
                Partition("test", 4, listOf(3, 1), listOf(3, 1), 3)
        ))
        ...
        describe("Replacing 1st node as it's down, 2 racks") {
            ...
            val strategy = ReplaceAbsentNodesPartitionAssignmentStrategy(
                    plan,
                    listOf(
                            KafkaBroker(2, "127.0.0.2", "2"),
                            KafkaBroker(3, "127.0.0.3", "3"),
                            KafkaBroker(4, "127.0.0.4", "1"),
                            KafkaBroker(5, "127.0.0.4", "2"),
                            KafkaBroker(6, "127.0.0.4", "3")
                    ),
                    MonoWeightFn(),
                    emptySet()
            )
            ...
            describe("new plan generated by strategy") {
                ...
                it("should contain all partition spread across nodes 2,3,4,5,6") {
                    assertThat(newPlan.partitions.map { it.replicas }.flatten().toSet()).intersects(setOf(2, 3, 4, 5, 6))
                }
